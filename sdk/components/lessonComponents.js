...
export function renderComplete(screen,recommendation,hasNext){
  const next = hasNext ? `<button class="nextCard" id="nextLessonBtn">${recommendation}</button>` : `<div class="coach" style="display:block">${recommendation}</div>`;
  return `<div class="complete"><div class="check">✓</div><span class="badge">Complete</span><h1>${screen.title}</h1><p>${screen.body}</p>${next}<button class="btn primary" id="restartBtn">Restart lesson</button></div>`;
}
