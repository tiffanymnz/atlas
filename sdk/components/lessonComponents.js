...
export function renderComplete(screen,recommendation,hasNext,copy={complete:"Complete",nextLesson:"Next lesson",reviewVisualGap:"Review visual gap",restartLesson:"Restart lesson"}){
  ...
  return `<div class="complete"><div class="check" aria-hidden="true">✓</div><span class="badge">${copy.complete}</span><h1 id="screenTitle">${screen.title}</h1><p>${screen.body}</p>${recommendation?`<p class="coach" role="status">${recommendation}</p>`:""}<div class="toolbar" style="justify-content:center"><button class="btn secondary" type="button" id="reviewBtn">${copy.reviewVisualGap}</button>${nextButton}<button class="btn secondary" type="button" id="restartBtn">${copy.restartLesson}</button></div></div>`;
}
