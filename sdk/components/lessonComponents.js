...
export function renderComplete(screen,recommendation,hasNext){
  ...
  return `<div class="complete"><div class="check">✓</div><span class="badge">Complete</span><h1>${screen.title}</h1><p>${screen.body}</p><div class="coach" style="display:block">${recommendation}</div><div class="toolbar" style="justify-content:center"><button class="btn secondary" id="reviewBtn">Review visual gap</button>${nextButton}<button class="btn secondary" id="restartBtn">Restart lesson</button></div></div>`;
}
