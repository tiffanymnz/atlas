...
test("memory assets are deterministic, bilingual, and traceable",async()=>{
  ...
  assert.equal(data.locales.en.length,11);
  assert.equal(data.locales.es.length,11);
  assert.notEqual(data.locales.en[0].title,data.locales.es[0].title);
  for(const locale of ["en","es"]){
    for(const lesson of data.locales[locale]){
      assert.match(lesson.lessonId,/^MATH-(?:NS-COMP-000[1-4]|NS-DIV-0005|NS-FRAC-0006|NS-PCT-0007|GEO-AREA-0008|GEO-PERIM-0009|ALG-INEQ-001[01])$/);
  ...
  assert.equal(manifest.lessonIds.length,11);
...
test("printable assets contain every lesson and no unresolved values",async()=>{
  ...
  for(const locale of ["en","es"]){
    for(const type of ["flashcards","worksheet","quiz"]){
      ...
      for(const id of ["MATH-NS-COMP-0001","MATH-NS-COMP-0002","MATH-NS-COMP-0003","MATH-NS-COMP-0004","MATH-NS-DIV-0005","MATH-NS-FRAC-0006","MATH-NS-PCT-0007","MATH-GEO-AREA-0008","MATH-GEO-PERIM-0009","MATH-ALG-INEQ-0010","MATH-ALG-INEQ-0011"]) assert.match(html,new RegExp(id));
